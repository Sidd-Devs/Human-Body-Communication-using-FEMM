# -- coding: utf-8 --
"""
Config-2: Tx-Ground electrode positioned ABOVE Tx-Signal electrode
"""
...
def run_with_params(params):
    ...
    # === CRITICAL CHANGE FOR CONFIG-2 ===
    # In Config-1: Tx-Ground is to the LEFT of Tx-Signal (x-direction)
    # In Config-2: Tx-Ground is ABOVE Tx-Signal (y-direction)
    ...
    rectangles = {
        ...
        # CONFIG-2: Tx-Signal is BELOW Tx-Ground (vertically stacked)
        # Tx-Signal sits on top of skin at y = 4.627
        "Tx-Signal": (70-TxD-TxW, 4.627, 70-TxD, 4.627+TxT),
        
        # Tx-Ground is ABOVE Tx-Signal, separated by gap TxG
        "Tx-Ground": (70-TxD-TxW, 4.627+TxT+TxG, 70-TxD, 4.627+TxT+TxG+TxT),

        # Rx electrodes remain the same
        "Rx-Signal": (55, 0, 70, 0-RxT),
        "Rx-Gnd": (55, 0-RxH-2*RxT, 55+RxW, 0-RxH-RxT),

        "Hub": (55, 0-RxT, 55+RxW, 0-RxT-RxH),

        "Wood": (55, 0-RxH-2*RxT, 90, -109),        
    ...
    femm.ei_saveas("temp_sim_config2.fee")
    femm.ei_analyze(1)
...
sweeps = {
    'TxD': list(range(10, 41, 5)),
    'TxW': list(range(1, 11, 1)),
    'RxH': list(range(10, 51, 10)),
    'RxW': list(range(5, 35, 5)),

    'TxG': list(range(1, 11, 1)),
}

output_file = "results_config2.csv"
